...
void Serialize(const transport_catalogue::TransportCatalogue& catalogue, const renderer::MapRenderer& renderer, const transport_catalogue::TransportRouter& router, std::ostream& output) {
    ...
    for (const auto& [name, s] : catalogue.GetSortedAllStops()) {
        *database.add_stop() = Serialize(s);
    }
    for (const auto& [name, b] : catalogue.GetSortedAllBuses()) {
...
void SetStopsDistances(transport_catalogue::TransportCatalogue& catalogue, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.stop_size(); ++i) {
        const serialize::Stop& stop_i = database.stop(i);
        transport_catalogue::Stop* from = catalogue.FindStop(stop_i.name());
        for (size_t j = 0; j < stop_i.near_stop_size(); ++j) {
            catalogue.SetDistance(from, catalogue.FindStop(stop_i.near_stop(j)), stop_i.distance(j));
        }
    }
}

void AddStopFromDB(transport_catalogue::TransportCatalogue& catalogue, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.stop_size(); ++i) {
        const serialize::Stop& stop_i = database.stop(i);
        catalogue.AddStop(stop_i.name(), { stop_i.coordinate(0), stop_i.coordinate(1) });
    }
    SetStopsDistances(catalogue, database);
}

void AddBusFromDB(transport_catalogue::TransportCatalogue& catalogue, const serialize::TransportCatalogue& database) {
    for (size_t i = 0; i < database.bus_size(); ++i) {
        const serialize::Bus& bus_i = database.bus(i);
        std::vector<transport_catalogue::Stop*> stops(bus_i.stop_size());
        for (size_t j = 0; j < stops.size(); ++j) {
            stops[j] = catalogue.FindStop(bus_i.stop(j));
        }
        catalogue.AddBus(bus_i.name(), stops, bus_i.is_roundtrip());
        if (!bus_i.final_stop().empty()) {
            transport_catalogue::Bus* bus = catalogue.FindBus(bus_i.name());
            bus->final_stop = catalogue.FindStop(bus_i.final_stop());
        }
    }
}
...
std::tuple<transport_catalogue::TransportCatalogue, renderer::MapRenderer, transport_catalogue::TransportRouter,
    ...
    transport_catalogue::TransportRouter router(GetRouterSettingsFromDB(database.router()));
    AddStopFromDB(catalogue, database);
    AddBusFromDB(catalogue, database);
    
    return { 
        std::move(catalogue), std::move(renderer), std::move(router), GetGraphFromDB(database.router()), GetStopIdsFromDB(database.router()) 
    };
}