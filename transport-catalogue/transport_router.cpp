...
namespace transport_catalogue {
    ...
    TransportRouter::TransportRouter(const json::Node& settings_node) {
        if (settings_node.IsNull()) return;
        SetSettings(settings_node);
    }

    TransportRouter::TransportRouter(const json::Node& settings_node, const TransportCatalogue& catalogue) {
        if (settings_node.IsNull()) return;
        SetSettings(settings_node);
    ...
    const graph::DirectedWeightedGraph<double>& TransportRouter::BuildGraph(const TransportCatalogue& catalogue) {
        const map<string_view, Stop*>& all_stops = catalogue.GetSortedAllStops();
        const map<string_view, Bus*>& all_buses = catalogue.GetSortedAllBuses();
            ...
            [&stops_graph, this](const auto& item) {
                ...
                for (size_t i = 0; i < stops_count; ++i) {
                    for (size_t j = i + 1; j < stops_count; ++j) {
                        ...
                        if (!bus_ptr->is_roundtrip && stop_to == bus_ptr->final_stop && j == stops_count / 2) break;
                    }
                }
            });
    ...
    size_t TransportRouter::GetGraphVertexCount() {
    ...
    json::Node TransportRouter::GetSettings() const {
    ...
    void TransportRouter::SetSettings(const json::Node& settings_node) {
